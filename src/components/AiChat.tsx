...
const N8N_CHAT_PROD_URL =
  "https://autonxt.app.n8n.cloud/webhook/1b0b4ec9-24d5-40e0-aced-f9d107f81a86/chat";
const N8N_WEBHOOK_URL = import.meta.env.DEV ? "/api/n8n-chat" : N8N_CHAT_PROD_URL;
...
export default function StaticChatBot() {
    ...
    try {
      ...
      const detail =
        error instanceof Error && error.message.includes("Chat service")
          ...
          : null;
      return detail
        ? `Sorry, chat is temporarily unavailable (${detail}). Please try again in a moment.`
  ...
  const openCallForm = (mode: "now" | "scheduled" = "now") => {
    ...
    setMessages((prev) => [
      ...
      {
        role: "assistant",
        text: mode === "scheduled" ? getScheduledCallPrompt(t) : getCallFormPrompt(userLanguage),
      },
    ]);
  };
  ...
  return (
    <>
      {/* FLOATING ACTIONS — chat only (call & meeting icons temporarily hidden) */}
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
        {/* <motion.button
          type="button"
          onClick={handleFloatingMeeting}
          ...
          whileTap={{ scale: 0.96 }}
        >
          <span className="hidden sm:block rounded-lg bg-blue-700 px-2.5 py-1 text-[10px] font-semibold text-white shadow-lg opacity-0 transition-opacity group-hover:opacity-100">
            Meeting
          </span>
          <span className="flex h-12 w-12 items-center justify-center rounded-full bg-blue-700 text-white shadow-xl ring-2 ring-blue-500/30">
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.96 }}
        >
          <span className="hidden sm:block rounded-lg bg-red-600 px-2.5 py-1 text-[10px] font-semibold text-white shadow-lg opacity-0 transition-opacity group-hover:opacity-100">
            Call
          </span>
          <span className="flex h-12 w-12 items-center justify-center rounded-full bg-red-600 text-white shadow-xl ring-2 ring-red-500/30">
        ...
        <motion.button
          ...
          className="group relative flex h-14 w-14 items-center justify-center rounded-full bg-black shadow-2xl"
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 z-50 w-[390px] max-w-[calc(100vw-24px)] h-[650px] max-h-[calc(100vh-8rem)] rounded-[34px] overflow-hidden bg-white shadow-2xl border border-white/10 flex flex-col"
          >
            {/* HEADER */}
            ...
            <div className="flex-1 overflow-y-auto bg-gray-50 px-4 py-5 space-y-4">
              ...
              {/* Call & meeting disabled — outbound call countdown */}
              {callCountdown !== null && (
                <motion.div
