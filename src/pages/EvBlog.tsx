...
import {
  ...
  TrendingUp, Leaf, IndianRupee, Search, ExternalLink, X, BookOpen, Tag, ChevronDown,
  SlidersHorizontal
} from "lucide-react";
...
export default function EvBlog() {
  ...
  const {
    ...
    searchFields: (post: any) => [post.title, post.summary],
  });
  ...
  const selectOptions = categories.map((category) => {
    ...
    return {
      ...
      label: category === "all" ? `${t.evBlog.allCategories} (${count})` : `${category as string} (${count})`,
    };
  });
  ...
  return (
    <div className="w-full min-h-screen bg-background">
              ...
              >
                {[
                  ...
                  <div key={i} className="flex items-center gap-2.5 group">
                    <div className="w-10 h-10 rounded-xl bg-muted border border-border flex items-center justify-center group-hover:bg-primary/20 group-hover:border-primary/30 transition-all duration-300">
                    ...
                    <div>
                      ...
                      <p className="text-foreground font-bold text-sm group-hover:text-primary transition-colors">{f.value}</p>
                    </div>
                  </div>
                ))}
              </motion.div>
      ...
      <section className="py-6 bg-muted/10 border-b border-border relative z-30">
      ...
      <section className="pt-10 pb-6 bg-muted/5">
        <div className="container mx-auto px-4 md:px-8 max-w-5xl">
          <ArticleSlider
            items={filteredPosts}
            noItemsMessage={t.evBlog.noArticlesFound}
            noItemsDesc={t.evBlog.noArticlesDesc}
            showAll={showAll}
